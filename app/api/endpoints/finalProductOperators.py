...
from app.services.finalProductOperators_services import Create_finalprodOperate, Find_by_OperatorID_FinalproductID, Get_FinalProductOperate
...
@router.get("/api/finalprodoperate/")
def get_finalprod_endpoint(db: Session = Depends(get_db)):
    result = Get_FinalProductOperate(db=db)
    return result
