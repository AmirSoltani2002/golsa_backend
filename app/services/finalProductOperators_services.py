...
from app.models import FinalProductOperator, Operator, FinalProduct, AllProduct, PipeProduct, FittingProduct
...
def Get_FinalProductOperate(db: Session, excel = True):
    ...
    if excel:
        for row in results:
            try:
                product = None
                if(row[12] == "لوله" and row[13]):
                    product = db.query(PipeProduct).filter(PipeProduct.code == row[13]).first()
                elif(row[12] == "اتصالات" and row[13]):
                    product = db.query(FittingProduct).filter(FittingProduct.code == row[13]).first()
